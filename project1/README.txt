...
# Computer Network Project 1 README #
...
What I have done:
	...
	Therefore, I use select() to get the client send connect requests or send message. If it is connect 
	request, I call accept() and add the fd into the select list. As for the message, server sends back 
	my protocol, which will be explained in follow, and print the requesting output. Last, if getting the 
	message of shutting down of client socket, server will delete the socket in its select list.
